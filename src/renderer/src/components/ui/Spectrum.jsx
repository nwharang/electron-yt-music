  ...
  initAnalyser = () => {
    ...
    function drawDots() {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        grd.addColorStop(0, 'hsla(' + (config.hueStart + npt * config.colorSpeed + dots[i].hue) + ', 86%, 63%, ' + avg / 400 + '%)')
        grd.addColorStop(1, 'hsla(' + (config.hueStart + npt * config.colorSpeed + dots[i].hue) + ', 86%, 63%, 0%)')
        ctx.fillStyle = grd
    ...
    let circleRotation = (Math.PI / 4) * 5
    ...
    function animationLooper() {
      ...
      radius = 100
      // image radius
      document.documentElement.style.setProperty('--image-width', radius + 140 + lowAvgFreq / (circleJumper / 2) + 'px')
      if (window.innerWidth <= 850) {
        // Change Image radius
        document.documentElement.style.setProperty('--image-width', radius + 65 + lowAvgFreq / (circleJumper / 2) + 'px')
      ...
      if (window.innerWidth <= 500) {
        // Change Image radius
        document.documentElement.style.setProperty('--image-width', radius + 60 + lowAvgFreq / (circleJumper / 2) + 'px')
        // Change Spectrum Radius
        radius = 70
      }
      let imageRadius = radius + 60 + lowAvgFreq / (circleJumper / 2)
      ctx.drawImage(image, centerX - imageRadius / 2, centerY - imageRadius / 2, imageRadius, imageRadius)

      for (var i = 0; i < bufferLength - 85; i++) {
        // - 85 because the low frequencies does not pick up for most songs
    ...
    const drawBar = (x1, y1, x2, y2, frequency, i, ctx) => {
      ...
      if (typeof this.props.currentColor === 'number') {
        const hue = i / 5 + this.props.currentColor
        ctx.strokeStyle = `hsl(${hue}, 84%, 63%)`
      } else {
